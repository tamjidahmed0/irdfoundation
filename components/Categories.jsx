...
//       <div>
//         <h1 className='text-[#393939] text-[24px] font-bold'>Dua Page</h1>
//       </div>
//       <div className='bg-white grid grid-rows-[122px_600px]  h-full rounded-xl'>

//         <div className='flex flex-col gap-3 sticky top-0 '>
//           <div className='bg-[#1FA45B] h-[57px] text-white font-semibold flex items-center justify-center rounded-t-xl'>
//             <h1>Categories</h1>
//           </div>
...
//                           {subCat[0]?.subCategories?.map((value, index) => (

//                             <AccordionItem value={value.id} key={index} id={search} onClick={() => handleScroll(search)}>

//                               <Link href={`/duas?cat=${search}&subcat=${value.subcat_id}`} onClick={()=> dispatch(setSubCatId(value.subcat_id))}>

//                                 <AccordionTrigger className=" p-[8px] space-y-3 " >
//                                   <div className="bg-[#1FA45B] -translate-x-3 mt-1.5 w-1.5 rounded-full h-1.5" />
//                                   <p className={`font-semibold text-[16px]  ${subcat == value.subcat_id ? 'text-[#1FA45B]' : 'text-[#373737]'}`}> {value.subcat_name_en}</p>
//                                 </AccordionTrigger>
...
//                               {value.duas?.map((value, index) => (
//                                 <Link href={`/duas?cat=${search}&subcat=${value.subcat_id}&dua=${value.dua_id}`} key={index} onClick={()=>dispatch(setDuaId(value.dua_id))}>
...
//                                       <p className={`ml-3 ${dua == value.dua_id ? 'text-[#1FA45B]' : 'text-[#373737]'}`}>{value.dua_name_en}</p>
//                                     </div>
//                                   </AccordionContent>
